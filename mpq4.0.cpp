...
{
	if (MessageBoxW(NULL, L"mpq4.0 has new GDI effects\nThis is just GDI no damages made.", L"mpq4.0.exe by PRGBQUAD-370", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL, L"Are u sure u still want to run it if not just press no", L"Final Warning - mpq4.0.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
		{
			ExitProcess(0);
		}
		else
